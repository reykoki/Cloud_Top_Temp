...
full_data_dir = '/scratch3/BMC/gpu-ghpcs/Rey.Koki/Cloud_Top_Temp/expert_classification/classification_data/'
...
def sample_doesnt_exist(yr, dn, fn_head):
    ...
    file_list = glob.glob('{}{}/{}/{}_{}_*.tif'.format(full_data_dir, yr, dn, sat_num, start_scan))
    if len(file_list) > 0:
...
def get_stats_dict(month, sat):
    ...
    for band in bands:
        ...
        pkl_loc = '../percentiles/stats/{}/{}/{}/percentile_stats_{}_{}_{}_{}.pkl'.format(year, sat, band, year, sat, band, str(month).zfill(2))
        print(pkl_loc)
...
def get_sample_list(goes_dl_loc, sat):
    ...
    if C14_list:
        ...
        for C14_sample in C14_list:
            ...
            if len(files_for_sample) == 7 and doesnt_exist:
                fn_head = C14_sample.split('C14_')[-1].split('.')[0].split('_e2')[0]
                #sample_dict = {"files_for_sample": files_for_sample, "fn_head": fn_head, "sat": sat, "year": yr, "dn": dn, "band_stats": band_stats}
                sample_dict = {"files_for_sample": files_for_sample, "fn_head": fn_head, "sat": sat, "year": yr, "dn": dn}
