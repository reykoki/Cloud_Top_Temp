...
def get_day_dts(dt):
    ...
    day_dts = [dt+timedelta(hours=t)+timedelta(minutes=random.randint(0, 59)) for t in range(24)]
    return day_dts
...
def GOES_doesnt_already_exists(yr, dn, fn_head):
    ...
    file_list = glob.glob('{}/{}/{}/{}*_{}_*.nc'.format(goes_dir, yr, dn, sat_num, start_scan))
    if len(file_list) > 0:
