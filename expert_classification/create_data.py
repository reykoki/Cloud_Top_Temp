...
def get_IR_stationary(scn, bands):
    C14 = scn[bands[0]].compute().data
    C14 = clip_scale_percentile(C14, 221.87, 312.43)
    C15 = scn[bands[1]].compute().data
    C15 = clip_scale_percentile(C15, 220.89, 307.48)
    C16 = scn[bands[2]].compute().data
    C16 = clip_scale_percentile(C16, 220.05, 281.96)
...
def split_and_save(full_image, full_coords, fn_head, img_size=1024):
    ...
    for row in range(n_row):
        for col in range(n_col):
            ...
            save_loc = '/scratch3/BMC/gpu-ghpcs/Rey.Koki/Cloud_Top_Temp/expert_classification/classification_data/'
            skimage.io.imsave('{}{}/{}/{}'.format(save_loc, yr, dn, fn), data)
            fn_list.append(fn)
    return fn_list
...
def create_data_truth(sat_fns, yr, dn, fn_head):#, band_stats):
    ...
    rgb_composite = 'cimss_true_color_sunz_rayleigh'
    rgb_scn = get_scn(sat_fns[0:-1], [rgb_composite], extent) # get satpy scn object
    temp_mask = get_temp(temp_scn)
    IR = get_IR_stationary(scn, bands) # from the scene object, extract RGB data for plotting
    corr_data = get_enhanced_image(rgb_scn[rgb_composite]).data.compute().data
    RGB = np.einsum('ijk->jki', corr_data)
    stacked = np.concatenate([RGB, IR, temp_mask], axis=-1)



    lon, lat = scn['C14'].attrs['area'].get_lonlats()
    lat_lon = np.dstack([lat, lon])
    tif_fns = split_and_save(stacked, lat_lon, fn_head, img_size=2048)
    #for sat_fn in sat_fns:
    #    os.remove(sat_fn)
    print(tif_fns)
